    ...
    MobileArea.prototype = {
        ...
        bindEvent: function() {
            ...
            function areaCtrlInit() {
                var provinceIndex = _self.value[0],cityIndex = _self.value[1],districtIndex = _self.value[2];
                _self.data.forEach(function(item, index){
                    if (item.id == _self.value[0]) {
                        provinceIndex = index;
                    }
                    item.child.forEach(function(cityitem, cIndex){
                        if (cityitem.id == _self.value[1]) {
                            cityIndex = cIndex;
                        }
                        // console.log(cityitem);
                        cityitem.child && cityitem.child.forEach(function(districtitem, dIndex){
                            if (districtitem.id == _self.value[2]) {
                                districtIndex = dIndex;
                            }
                        });
                    });
                });
                _self.gearArea.querySelector(".area_province").setAttribute("val", provinceIndex);
                _self.gearArea.querySelector(".area_city").setAttribute("val", cityIndex);
                _self.gearArea.querySelector(".area_county").setAttribute("val", districtIndex);
                switch (_self.type) {
                case 1:
