...
/* WEBPACK VAR INJECTION */(function($) {/*global ZegoClient*/
...
function setSdk(type) {
    if (type) {
        // 同步版本  适用于性能好的机器
        window.quality = true;
        ZegoClient = __webpack_require__(27);
    } else {
        // 异步版本  适用于性能不好的机器  视频将会码率降级
        window.quality = false;
        ZegoClient = __webpack_require__(26);
    }
}
// 工具函数
...
var $roomMoney = $(".recharge-open"); //充值按钮
var $rechargewrapper = $("#recharge-wrapper"); //充值列表
var $wawaDetailsBtn = $(".wawa-details-btn"); //娃娃详情按钮
var $wawaDetails = $("#wawaDetails"); //详情列表框
var $detailsCloseBtn = $(".detailsCloseBtn"); //详情列表框关闭按钮
...
var appid = 1016805126; //appid
var roomID = g.getUrlParams("rid"); //房间id
var roomCode = g.getUrlParams("rcode"); //房间code
var dollType = g.getUrlParams("dollType"); //dollType                                                    
console.log('roomid = ', roomID, '\n');
console.log('roomCode = ', roomCode, '\n');
console.log('dollType = ', dollType, '\n');
var userCode = g.getSession("uCode"); //用户code
...
var server = 'ws://175.6.223.31:8090'; // wawaji接入服务器地址
var logUrl = ''; // log服务器地址 ---
var loginTokenUrl = config.apihost + '/h5/token'; // 登录token派发地址
//var payTokenUrl = config.apihost+'/room/zegoPay'; // 支付地址 ---
...
window.onload = function () {
    ...
    function updateStreamInfo(streamList, updateType) {
        ...
        if (useStreamList.length !== 0) {
            useStreamList.forEach(function (item) {
                ...
                if (item.stream_id.slice(-5) == '_lowB') {
    ...
    $switchBtn.addEventListener('click', function (e) {
        ...
        zg.sendRoomMsg(2, 1, v, function (seq, msdid, msgcategory, msgtype, msgcontent) {
    ...
    */
    ...
    $apply.addEventListener('click', reservationQueuing);
     ...
     * @param roomCode:{String} => 房间code
     */
    function reservationQueuing() {
        var requestParams = '{"userCode":"' + userCode + '","roomCode":"' + roomCode + '",' + g.tempOpt() + '"}';
        var methodName = "/room/reservationQueuing";
        ...
        g.request({
            ...
            if (res.status === 1) {
                ...
                var userCoin = parseInt(res.userCoin);
                $("#balance").html(userCoin);
            } else {
                tips("请充值");
            }
        });
    ...
    function pay() {
        ...
        roomzegoPay(sessionID, payTimestamp, userCode, roomCode);
        // var xmlhttp;
        // xmlhttp = new XMLHttpRequest();
        // xmlhttp.onreadystatechange = function() {
        //     if (xmlhttp.readyState == 4) {
        //         if (xmlhttp.status == 200) { // 此处是要业务后台自己写判断逻辑，根据业务后台返回的信息来 断定是否支付成功
        //             getpayToken = xmlhttp.responseText;
        //             payToken = getpayToken.token;
        ...
        // xmlhttp.open("GET", payTokenUrl + "?session_id=" + sessionID + "&confirm=1" + "&time_stamp=" + payTimestamp + "&userCode=" + userCode + "&roomCode=" + roomCode, true);
        // xmlhttp.send();
    }
    // 支付确认返回
    function roomzegoPay(session_id, time_stamp, userCode, roomCode) {
        var requestParams = '{"session_id":"' + session_id + '","confirm":"1","time_stamp":"' + time_stamp + '","userCode":"' + userCode + '","roomCode":"' + roomCode + '"}';
        var methodName = "/room/zegoPay";
        var str = JSON.parse(requestParams);
        g.request({
            "url": methodName,
            "param": str
    ...
    //娃娃机详情框
    touchend($wawaDetailsBtn, function (e, _this) {
        e.stopPropagation();
        e.preventDefault();
        var requestParams = '{"userCode":"' + userCode + '","dollType":"' + dollType + '",' + g.tempOpt() + '"}';
        if (!$wawaDetails.hasClass("is-load")) {
            roomDetailPic(requestParams);
        }
        $wawaDetails.addClass("is-visible");
    });
    //娃娃机详情框关闭
    touchend($detailsCloseBtn, function (e, _this) {
        e.stopPropagation();
        e.preventDefault();
        $wawaDetails.removeClass("is-visible");
    });
};
 ...
 * @param userCode:{String} => 用户信息
 */
function roomDetailPic(params) {
    var methodName = "/room/roomDetailPic";
    ...
    g.request({
        ...
        if (res.status === 1) {
            $("#wawa-imgs").html("");
            $wawaDetails.addClass("is-load");
            var data = res.detailPicList;
            if (data.length > 0) {
                var html = '';
                $.each(data, function (k, v) {
                    html += '<li>';
                    html += '<img src="' + v.picUrl + '" alt="">';
                    html += '</li>';
                });
                $("#wawa-imgs").append(html);
            } else {
                tips("无列表");
            }
        } else {
...
function getRoomUserList() {
    //util.removeClass($anchorWrapper, 'is-visible');
    var requestParams = '{"userCode":"' + userCode + '","roomCode":"' + roomCode + '","timestamp2":"0",' + g.tempOpt() + '"}';
    var methodName = "/barrage/get";
    var str = JSON.parse(requestParams);
    $.ajax({
        url: config.apihost + methodName,
        type: "post",
        data: str,
        async: true,
        success: function (data) {
            var lookuserlist = data.LookUserList;
            $(".audience-wrapper").addClass('is-visible');
            if (lookuserlist.length > 0) {
                var html = '';
                $.each(lookuserlist, function (k, v) {
                    html += '<li class="avatar">';
                    html += '<img src="' + v.picUrl + '" alt="">';
                    html += '</li>';
                });
                $(".audience-ul").html(html);
                $audience.innerHTML = data.LookUserCount;
            }
            var PlayUser = data.PlayUser;
            if (parseInt(PlayUser.gameStatus)) {
                $(".anchor-wrapper").addClass('is-visible');
                $anchor.innerHTML = PlayUser.userName;
                $("#userImg").attr("src", PlayUser.picUrl);
            } else {
                $(".anchor-wrapper").removeClass('is-visible');
            }
        },
        error: function (err) {
            tips(err);
...
function getUserInfo() {
    ...
    g.request({
        ...
        if (res.status === 1) {
            ...
            util.setLocal('idName', data.nickName);
            queryCoin(0);
            $("#userinfo").html('<img id="user-img" src="' + data.picUrl + '" alt=""><p>' + data.nickName + '</p><div class="cur-money"><img src="../static/images/ico_gold.png"><div id="balance">' + data.coin + '</div></div>');
        } else {
            tips(res.msg);
        }
    });
 ...
 * @param userCode:{String} => 用户信息
 */
function queryCoin(k) {
    var requestParams = '{"userCode":"' + userCode + '","roomCode":"' + roomCode + '",' + g.tempOpt() + '"}';
    var methodName = "/room/queryCoin";
    var str = JSON.parse(requestParams);
    g.request({
        "url": methodName,
        "param": str
        ...
        if (!k) {
            setInterval(getRoomUserList, 6000);
        }
        if (res.status === 1) {
            var gameCoin = parseInt(res.gameCoin);
            $("#price").html(gameCoin);
            $("#balance").html(parseInt(res.userCoin));
        }
    });
}
...
// 左上角当前主播状态样式
//require('@scss/wawaji/anchor.scss');

// 观众
//require('@scss/wawaji/audience_num-network.scss');
...
// 日志样式
//require('@scss/wawaji/log-view.scss');
